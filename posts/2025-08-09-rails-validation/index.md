...
## 補足: 意味的妥当性とドメインロジック
上述した意味的妥当性のための検証は、ドメインロジックとして捉えるべきという考えもあるっぽい（セキュアバイデザインP166を参照）
...
```rub
  ...
  private
    ...
    def validate_admin_changes
      errors.add(:date, message: '管理者は日付を変更できません') if will_save_change_to_date?
    end

```

なぜなら、「責務の混在」が発生する。
モデルが「誰が操作するか」を知ると、テスト/保守が重くなる（文脈増加で分岐が増殖）。
データ整合性のためのバリデーションに認可の話が混ざり込んでしまっている。

Policyを使っていない段階であるなら、呼び出し側でパラメータをフィルタして“更新させない”実装のほうが良い。
ただし、入口が分散していてフィルタ適用漏れが現実的に起こりうるデメリットがあるので、理想的にはPdMとなどを用いて一元管理したいところではある。

好きではないが、許容する理由としては、「ドメイン不変条件（いつ/どんな状態なら不可）として表現できるなら、モデルに置く価値が高い。」という観点かもしれない。

とはいえ、できるだけバリデーションには認可の話をわたしは持ち込みたくない派閥である
