...
そもそも、PunditはModelのドメインロジックから切り離された存在であるため、ドメインロジックに依存してはならない
...
この前提に立つと、UserPolicy はあくまでUser モデル（アプリケーション内のユーザー情報）に対する操作に責任を持つべきであるはず。
...
パスワードなどの認証情報はFirebase側で管理されており、Userモデルとは責務が分離されている。

よって、UserPolicyの責務は、アプリケーション内のUserリソースに対するアクセス制御に限定されるべきで
...
## 追記
User モデルに仮想属性 password を持たせ、Policy 側で「パスワードの有無」を基準に認可可否を判定する設計は、Policyの責務を超えているとはいえない。

理由としては、

- Userのpasswordカラムの有無は「状態の確認」に過ぎず、認可判断のトリガーとして機能している
- これはあくまで User の状態に基づいた認可の設計であり、ポリシー側がドメインルールを内包しているとは言えない

逆に、これが Hash や構造体などの入力値そのものに依存する設計であれば、コントローラとの密結合を生み、ドメインロジックをポリシーに取り込んでいる状態となるため望ましくない

よって、password のような仮想属性によるシンプルな状態判定であれば、設計上の分離は保たれており、実用的かつ許容されるアプローチと言える。
